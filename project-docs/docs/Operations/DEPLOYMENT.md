...
and writes the GitHub release body to:

```text
RELEASE_NOTES.txt
```

`RELEASE_NOTES.txt` is customer-facing. Engineering notes are written to:
...
and release note artifacts after merges to `main`. The action does not commit
...
`main`, run the signed release from a clean and up-to-date `main`, then let the
Release Notes action generate artifacts from merged PRs since the previous
version tag. The signed release consumes those artifacts instead of regenerating
notes locally.
