...
stay local. Release notes, version preparation, and app-version publication are
moving into GitHub Actions so source changes remain PR-reviewable.
...
The Publish App Version workflow waits until `app-release.apk` is reachable on
the GitHub Release, then writes `appConfig/version`. This prevents users from
seeing an update notification before the APK exists.

Web deployment is handled separately by the Deploy Web workflow after `Build
Check` succeeds on `main`. `build:signed` does not deploy GitHub Pages.
...
The signed APK helper runs `yarn check:asset-budgets` after the mobile web
build. Asset budget target misses up to 15% over target pass with a warning and
make frontend optimization a top priority for the next release. Asset budget
misses more than 15% over target block the release before Capacitor sync or APK
upload proceeds.
