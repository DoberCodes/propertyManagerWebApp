...
exports.adminPortalMutateMaintleyTeam = exports.adminPortalListMaintleyTeam = void 0;
...
const displayNameForUser = (user) => `${String(user.firstName || '').trim()} ${String(user.lastName || '').trim()}`.trim() ||
    String(user.email || '').trim() ||
    'Maintley team member';
exports.adminPortalListMaintleyTeam = functions
    .region('us-central1')
...
exports.adminPortalMutateMaintleyTeam = functions
    .region('us-central1')
    ...
    try {
        ...
        const eventId = (0, entitlements_1.getAdminAuditEventId)(auditAction, requestId);
        ...
        let invitationEmailOutcome = 'not_applicable';
        if (action === 'invite') {
            try {
                ...
                const html = (0, emailBrand_1.renderMaintleyEmailShell)({
                    ...
                    bodyHtml: `
						<p style="margin:0 0 16px; font-size:15px; line-height:1.7; color:${emailBrand_1.EMAIL_BRAND.slate};">You were added as <strong>${(0, emailService_1.escapeHtml)(nextRole)}</strong>.</p>
						<p style="margin:0 0 18px; font-size:15px; line-height:1.7; color:${emailBrand_1.EMAIL_BRAND.slate};">Use the secure link below to ${createdAuthUser ? 'set your password and sign in' : 'open Maintley'}.</p>
						<a href="${(0, emailService_1.escapeHtml)(signInLink)}" style="display:inline-block; background:${emailBrand_1.EMAIL_BRAND.primary}; color:${emailBrand_1.EMAIL_BRAND.white}; text-decoration:none; padding:12px 18px; border-radius:9px; font-size:14px; font-weight:800;">Open Maintley</a>
					`,
                ...
                const failureEventId = (0, entitlements_1.getAdminAuditEventId)('admin_action.failed', failureRequestId);
