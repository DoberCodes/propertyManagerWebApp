...
exports.extractPdfDocument = void 0;
const loadPdfJs = () => {
...
const rowText = (items) => items
    ...
    .trim();
const groupItemsIntoLines = (items) => groupItemsIntoRows(items)
    .map((row) => rowText(row.items))
    .filter(Boolean);
const extractKnownTables = (items) => {
    ...
    for (let headerIndex = 0; headerIndex < rows.length; headerIndex += 1) {
        ...
        const table = [labels.map((label) => label.replace(/\b\w/g, (char) => char.toUpperCase()))];
        let current = null;
        let leadingNotes = [];
        const finishCurrent = () => {
            if (current && current.some(Boolean))
                table.push(current.map((cell) => cell.trim()));
            current = null;
        };
        for (let rowIndex = headerIndex + 1; rowIndex < rows.length; rowIndex += 1) {
            const row = rows[rowIndex];
            const text = rowText(row.items);
            if (/^(photos from visit|it is our mission|summary of|maintenance tasks|status checks|description photo)/i.test(text))
                break;
            if (rows[headerIndex].y - row.y > 610)
                break;
            const cells = ['', '', ''];
            ...
            if (!current) {
                if (cells[2])
                    leadingNotes.push(cells[2]);
                continue;
            }
            cells.forEach((cell, column) => {
                if (cell)
                    current[column] = [current[column], cell].filter(Boolean).join('\n');
            });
        }
        finishCurrent();
        if (table.length > 1)
...
const extractPdfDocument = async (buffer) => {
...
exports.extractPdfDocument = extractPdfDocument;
