...
exports.releaseStorageQuotaUsage = exports.finalizeStorageQuotaUsage = exports.getStorageQuotaStatus = exports.reserveStorageUpload = void 0;
...
const enabled = () => process.env.ENABLE_TRUSTED_STORAGE_QUOTA === 'true';
const text = (value) => String(value || '').trim();
const assertStoragePathForAccount = async (storagePath, accountId, uid) => {
    if (storagePath.startsWith(`properties/${accountId}/`) ||
        storagePath.startsWith(`team-member-images/${accountId}/`) ||
        storagePath.startsWith(`team-member-files/${accountId}/`))
        return;
    if (storagePath.startsWith(`user-profile-images/${uid}/`) && accountId === uid)
        return;
    ...
    if (['device-files', 'maintenance-files'].includes(parts[0]) && parts[1]) {
        ...
        if (property.exists && text(property.data()?.accountId) === accountId)
            return;
    }
    throw new functions.https.HttpsError('permission-denied', 'The upload path is outside this account.');
};
const resolveAccountIdForStorageObject = async (name, metadata) => {
    ...
    if (parts[0] === 'user-profile-images' && parts[1]) {
        return (0, accountAuthz_1.resolveAccountIdForUser)(parts[1]);
    }
    if (['device-files', 'maintenance-files'].includes(parts[0]) && parts[1]) {
        const property = await db.collection('properties').doc(parts[1]).get();
...
const measureAccountStorage = async (accountId) => {
    ...
    const files = (await Promise.all(prefixes.map((prefix) => bucket.getFiles({ prefix })))).flatMap(([items]) => items);
...
exports.reserveStorageUpload = functions.region('us-central1').https.onCall(async (data, context) => {
    if (!context.auth?.uid)
        throw new functions.https.HttpsError('unauthenticated', 'Sign in to upload files.');
    if (!enabled())
        throw new functions.https.HttpsError('failed-precondition', 'Trusted storage quota is disabled.');
    ...
    const maxFiles = (0, entitlements_1.getEntitlementLimit)(entitlements, 'files');
    const maxBytes = (0, entitlements_1.getEntitlementLimit)(entitlements, 'storage_gb') * BYTES_PER_GB;
...
exports.getStorageQuotaStatus = functions.region('us-central1').https.onCall(async (_data, context) => {
    if (!context.auth?.uid)
        throw new functions.https.HttpsError('unauthenticated', 'Sign in to view storage usage.');
    if (!enabled())
        throw new functions.https.HttpsError('failed-precondition', 'Trusted storage quota is disabled.');
    const accountId = await (0, accountAuthz_1.resolveAccountIdForUser)(context.auth.uid);
    await (0, accountAuthz_1.assertAccountRole)(context.auth.uid, accountId, ['account_owner', 'admin', 'manager']);
    ...
    return {
        ...
        maxFiles: (0, entitlements_1.getEntitlementLimit)(entitlements, 'files'),
        maxBytes: (0, entitlements_1.getEntitlementLimit)(entitlements, 'storage_gb') * BYTES_PER_GB,
    };
});
exports.finalizeStorageQuotaUsage = functions.storage.object().onFinalize(async (object) => {
...
exports.releaseStorageQuotaUsage = functions.storage.object().onDelete(async (object) => {
