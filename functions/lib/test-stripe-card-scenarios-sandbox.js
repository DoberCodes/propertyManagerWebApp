...
const assertEnv = () => {
    if (!stripeSecretKey) {
        throw new Error('Missing STRIPE_SECRET_KEY (or STRIPE_TEST_SECRET_KEY). Add your Stripe test secret key.');
    }
    if (!stripeSecretKey.startsWith('sk_test_')) {
        throw new Error('STRIPE_SECRET_KEY must be a Stripe TEST key (sk_test_...) for card scenario testing.');
    }
};
const scenarios = [
    {
...
const runScenario = async (stripe, scenario) => {
    try {
        ...
        if (scenario.expectation.outcome === 'succeeded') {
            if (paymentIntent.status !== 'succeeded') {
                throw new Error(`Expected succeeded, got ${paymentIntent.status} for ${scenario.name}`);
            }
        ...
        if (scenario.expectation.outcome === 'requires_action') {
            if (paymentIntent.status !== 'requires_action') {
                throw new Error(`Expected requires_action, got ${paymentIntent.status} for ${scenario.name}`);
            }
            console.log(`✅ ${scenario.name}: requires_action as expected`);
            return;
        }
        throw new Error(`Expected card_error for ${scenario.name}, but PaymentIntent was created with status ${paymentIntent.status}`);
    }
    catch (error) {
        ...
        const errorCode = (stripeErr === null || stripeErr === void 0 ? void 0 : stripeErr.code) || '';
        const expectedCode = scenario.expectation.code;
        if (expectedCode && errorCode !== expectedCode) {
            throw new Error(`Expected error code ${expectedCode}, got ${errorCode || 'unknown'} for ${scenario.name}`);
        }
        console.log(`✅ ${scenario.name}: card_error as expected (${errorCode || 'no code'})`);
    }
};
const run = async () => {
    assertEnv();
