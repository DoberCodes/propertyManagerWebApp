...
const normalizeAssetType = (value) => {
    ...
    const exactMatch = ASSET_TYPE_DEFINITIONS.find((definition) => normalize(definition.value) === normalizedValue);
    if (exactMatch)
...
const inferAssetVariantFromText = (assetType, text) => {
    ...
    if (normalize(assetType) === normalize('HVAC')) {
        if (normalizedText.includes('central air') ||
            normalizedText.includes('ac unit') ||
            normalizedText.includes('air conditioner')) {
            return 'Central AC';
        }
        if (normalizedText.includes('boiler'))
            return 'Boiler';
        if (normalizedText.includes('radiator'))
            return 'Radiator';
        if (normalizedText.includes('geothermal') || normalizedText.includes('geo')) {
            return 'Geothermal';
        }
        if (normalizedText.includes('air handler'))
            return 'Air Handler';
    }
    if (normalize(assetType) === normalize('Water Heater')) {
        ...
        if (normalizedText.includes('tankless') &&
            (normalizedText.includes('electric') || normalizedText.includes('electrical'))) {
            return 'Tankless Electric';
        }
        if (normalizedText.includes('tankless'))
            return 'Tankless Gas';
        if (normalizedText.includes('heat pump'))
            return 'Heat Pump';
        if (normalizedText.includes('solar'))
            return 'Solar';
        if (normalizedText.includes('electric') || normalizedText.includes('electrical')) {
            return 'Tank Electric';
        }
        if (normalizedText.includes('gas'))
            return 'Tank Gas';
    }
    if (normalize(assetType) === normalize('Refrigerator')) {
        if (normalizedText.includes('french door'))
            return 'French Door';
        if (normalizedText.includes('side by side'))
            return 'Side-by-Side';
        if (normalizedText.includes('bottom freezer'))
            return 'Bottom Freezer';
        if (normalizedText.includes('top freezer'))
            return 'Top Freezer';
        if (normalizedText.includes('built in'))
            return 'Built-In';
    }
    if (normalize(assetType) === normalize('Safety Device')) {
        if (normalizedText.includes('combo') ||
            (normalizedText.includes('smoke') && normalizedText.includes('carbon monoxide'))) {
            return 'Combo Detector';
        }
        if (normalizedText.includes('smoke'))
            return 'Smoke Detector';
        if (normalizedText.includes('carbon monoxide') ||
...
const deriveAssetFields = (device) => {
    ...
    const assetVariant = String(device.assetVariant || '').trim() ||
...
const run = async () => {
    console.log(`Starting device asset backfill in ${APPLY_CHANGES ? 'apply' : 'dry-run'} mode...`);
    ...
    for (const docSnapshot of snapshot.docs) {
        ...
        if (nextFields.assetVariant !== undefined &&
    ...
    console.log(`Device asset backfill completed. inspected=${inspected}, changed=${changed}, skipped=${skipped}, mode=${APPLY_CHANGES ? 'apply' : 'dry-run'}`);
};
run()
    .then(() => process.exit(0))
    .catch((error) => {
