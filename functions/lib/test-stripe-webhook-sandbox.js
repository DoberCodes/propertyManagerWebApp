...
const assertEnv = () => {
    ...
    if (!stripeSecretKey.startsWith('sk_test_')) {
        throw new Error('STRIPE_SECRET_KEY must be a Stripe TEST key (sk_test_...) for sandbox webhook testing.');
    }
    if (!webhookSecret) {
        throw new Error('Missing STRIPE_WEBHOOK_SECRET. Add your Stripe webhook signing secret from test mode.');
    }
    if (!webhookUrl) {
        throw new Error('Missing STRIPE_WEBHOOK_TEST_URL. Set it to your webhook endpoint (for example: https://us-central1-<project>.cloudfunctions.net/stripeWebhook).');
    }
};
const run = async () => {
    assertEnv();
    ...
    if (!response.ok) {
        throw new Error(`Webhook endpoint returned ${response.status}. Body: ${responseText}`);
    }
    if (!responseText.includes('received')) {
        console.warn('⚠️ Webhook endpoint responded without explicit "received" payload. Verify logs for event handling details.');
    }
