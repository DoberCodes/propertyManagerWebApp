...
exports.personalAssistantApi = void 0;
...
const pageSize = (value) => Math.max(1, Math.min(MAX_PAGE_SIZE, numberParam(value, 50)));
const encodeCursor = (id) => Buffer.from(JSON.stringify({ id }), 'utf8').toString('base64url');
...
const authenticate = async (req) => {
    ...
    const parsed = (0, personalAssistantCredentialCore_1.parsePersonalAssistantToken)(match?.[1] || '');
    if (!parsed)
        throw new ApiError(401, 'invalid_token', 'Authentication failed.');
    ...
    if (!snapshot.exists ||
        !data ||
        data.status !== 'active' ||
        !(0, personalAssistantCredentialCore_1.tokenVerifierMatches)(match[1], TOKEN_PEPPER.value(), data.tokenVerifier)) {
        throw new ApiError(401, 'invalid_token', 'Authentication failed.');
    }
...
const requireScope = (credential, scope) => {
...
const mapMaintenanceEvent = (snapshot) => {
    const data = snapshot.data() || {};
    return {
        id: snapshot.id,
        propertyId: String(data.propertyId || ''),
...
const collectionPage = async (collection, propertyId, cursor, limit) => {
...
const maintenancePage = async (propertyId, cursor, limit) => {
...
const filterByDatesAndStatus = (items, query) => {
    ...
    return items.filter((item) => {
        ...
        if (to && eventDate && eventDate > to)
            return false;
        if (updatedAfter && item.updatedAt && item.updatedAt <= updatedAfter)
            return false;
        return true;
    });
};
const latestInsights = async (propertyId) => {
    ...
    return (Array.isArray(data.recommendations) ? data.recommendations : []).slice(0, 100).map((item) => ({
        id: String(item.id || ''),
        propertyId,
...
const routeRequest = async (req, credential) => {
...
exports.personalAssistantApi = functions
    .region('us-central1')
    ...
    try {
        if (req.method !== 'GET')
            throw new ApiError(405, 'read_only_api', 'Only GET requests are supported.');
        ...
        scope = result.scope;
        status = 200;
        resultCount = Array.isArray(result.body.data) ? result.body.data.length : 1;
        ...
        if (!(error instanceof ApiError))
            functions.logger.error('Personal assistant API failure', { requestId, error });
        res.status(status).json({ error: { code: apiError.code, message: apiError.message, requestId } });
    }
    finally {
        if (credential) {
            const now = admin.firestore.Timestamp.now();
            const writes = [db.collection(AUDITS).add({
                    requestId,
