...
const assertEnv = () => {
    ...
    if (!stripeSecretKey.startsWith('sk_test_')) {
        throw new Error('STRIPE_SECRET_KEY must be a Stripe TEST key (sk_test_...) for sandbox testing.');
    }
    if (!stripePriceId) {
        throw new Error('Missing test price id. Set STRIPE_TEST_PRICE_ID or REACT_APP_STRIPE_*_PLAN_ID.');
    }
};
const run = async () => {
    assertEnv();
    ...
    try {
        ...
        if (customerId) {
            try {
                ...
                console.warn(`⚠️ Could not delete test customer ${customerId}. Manual cleanup may be required.`);
