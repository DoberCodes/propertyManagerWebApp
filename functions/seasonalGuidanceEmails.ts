...
interface SeasonalGuidanceUser {
	email?: string;
	firstName?: string;
	displayName?: string;
...
const SEASONAL_TIPS: Record<Season, SeasonalTip[]> = {
	spring: [
		{
...
const getDisplayName = (user: SeasonalGuidanceUser): string => {
	const name = (user.firstName || user.displayName || '').trim();
	return name || 'there';
};

const getSeasonForDate = (date: Date): Season =>
	SEASON_BY_MONTH[date.getMonth()] || 'spring';

const getSeasonLabel = (season: Season): string =>
	season.charAt(0).toUpperCase() + season.slice(1);

const getSeasonalGuidanceHtml = ({
	name,
	season,
	appUrl,
}: {
				...
				<tr>
					<td style="padding:0 0 14px 0;">
						<div style="border:1px solid #dbe7dc; border-radius:16px; padding:18px; background:#ffffff;">
							<p style="margin:0 0 8px 0; font-size:16px; font-weight:900; color:#10251a;">${escapeHtml(tip.title)}</p>
							<p style="margin:0; font-size:14px; line-height:1.65; color:#405348;">${escapeHtml(tip.body)}</p>
						</div>
					</td>
				</tr>
			`,
			...
			<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#edf7ef; padding:34px 14px;">
				<tr><td align="center">
					<table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px; width:100%; background:#ffffff; border-radius:20px; overflow:hidden; border:1px solid #cfe8d4; box-shadow:0 10px 30px rgba(16,37,26,0.08);">
						<tr>
							<td style="background:#0f5132; color:#ffffff; padding:30px 32px;">
						...
						<tr><td style="padding:32px;">
							<p style="margin:0 0 22px 0; font-size:16px; line-height:1.65; color:#33443a;">Hi ${escapeHtml(name)}, as ${seasonLabel.toLowerCase()} begins, here are a few common property areas homeowners often choose to review or document in Maintley.</p>
							<div style="font-size:12px; text-transform:uppercase; letter-spacing:0.08em; font-weight:900; color:#0f5132; margin-bottom:12px;">Seasonal prompts</div>
							...
							<a href="${escapeHtml(appUrl)}" style="display:inline-block; background:#13a85b; color:#ffffff; text-decoration:none; padding:13px 20px; border-radius:12px; font-size:14px; font-weight:900;">Open Maintley</a>
						</td></tr>
...
const sendSeasonalGuidanceForUser = async (
	userId: string,
	season: Season,
	appUrl: string,
): Promise<SendSeasonalResult> => {
	const userDoc = await db.collection('users').doc(userId).get();
	if (!userDoc.exists) {
		return { sent: false, skipped: true, reason: 'user_not_found' };
	}

	const user = userDoc.data() as SeasonalGuidanceUser;
...
export const sendSeasonalGuidanceEmails = functions
	...
	.onRun(async () => {
		...
		for (const userDoc of usersSnapshot.docs) {
			try {
				const result = await sendSeasonalGuidanceForUser(
					userDoc.id,
					season,
					appUrl,
				);
...
export const sendSeasonalGuidanceEmailTest = functions
	...
	.https.onCall(async (_data: Record<string, never>, context) => {
		if (!context.auth?.uid) {
			throw new functions.https.HttpsError(
				'unauthenticated',
				'You must be signed in to send a test seasonal guidance email.',
			);
		}
		...
		try {
			...
			if (result.skipped) {
				throw new functions.https.HttpsError(
					'failed-precondition',
					`Test seasonal guidance email was skipped: ${result.reason || 'unknown_reason'}`,
				);
			}
