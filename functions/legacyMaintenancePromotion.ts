...
const firstString = (record: LegacyRecord, fields: string[]): string => {
...
const stripUndefined = (value: unknown): any => {
...
const sourceHash = (legacy: LegacyRecord): string =>
	createHash('sha256').update(JSON.stringify(legacy)).digest('hex');

const normalizeAttachments = (legacy: LegacyRecord) => {
	...
	const normalized = attachments
		.map((attachment: LegacyRecord) => ({
			...
			mimeType:
				toString(attachment?.mimeType || attachment?.type) ||
...
const normalizeFinancials = (legacy: LegacyRecord) => {
	...
	if (!Number.isFinite(actualCost) && Object.keys(actual).length === 0) return undefined;
...
const normalizeRecordedBy = (legacy: LegacyRecord) => {
...
const normalizePerformedBy = (legacy: LegacyRecord) => {
...
export const buildPromotedLegacyMaintenanceEvent = ({
	legacyId,
	legacy,
	accountId,
	propertyId,
	...
	const deviceIds = dedupeStrings(
		legacy.deviceIds,
		legacy.deviceId,
		legacy.assignedDeviceId,
	);
	const linkedTaskIds = dedupeStrings(
		legacy.linkedTaskIds,
		legacy.originalTaskId,
		legacy.linkedTaskId,
		legacy.taskId,
		legacy.assignedTaskId,
	);
