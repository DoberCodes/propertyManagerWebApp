...
const cleanupInvalidPushToken = async (userId: string, pushToken: string) => {
	try {
...
export const sendPushForNotification = async (
	...
	if (
		...
		console.log(
			`Notification type '${notificationType}' is disabled for user ${userId}`,
		);
		return;
	}
	...
	try {
		...
		console.log(
			`Push delivery for user ${userId}: ${response.successCount} succeeded, ${response.failureCount} failed`,
		);
